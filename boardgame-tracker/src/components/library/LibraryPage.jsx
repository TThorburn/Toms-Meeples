...
function ShelfPlank({ topTexture, frontTexture }) {
  return (
    <div
      ...
      style={{ height: SHELF_HEIGHT, zIndex: 2 }}
    >
      {/* Shelf top */}
      <div
        style={{
          height: SHELF_TOP_HEIGHT,
          background: `url(${topTexture})`,
          backgroundRepeat: 'repeat',
          backgroundSize: 'auto 50px',
          boxShadow:
            'inset 0 1px 0 rgba(0,0,0,0.25), inset 0 -1px 0 rgba(168, 152, 86, 0.25)',
        }}
      />
      ...
      <div
        style={{
          height: SHELF_HEIGHT - SHELF_TOP_HEIGHT,
          background: `url(${frontTexture})`,
          backgroundRepeat: 'repeat',
          backgroundSize: 'auto 50px',
          boxShadow: `
            inset 0 1px 0 rgba(95, 83, 58, 0.25),
            inset 0 -1px 0 rgba(34,23,10,0.28)
          `,
        }}
      />
    </div>
...
function GameBox({ item, onRemove, onSelect, removing }) {
  ...
  return (
    <motion.div layout className="relative" style={{ width: boxWidth, zIndex: 3 }}>
      ...
      >
        <img
          src={item.image || placeholderGame}
          alt={item.name}
...
function ShelfRow({ items, onRemove, onSelect, removing }) {
  const slots = [...items]
  while (slots.length < BOOKS_PER_SHELF) slots.push(null)
  ...
  return (
    ...
    >
      <div
        className="flex items-end gap-3 px-5 pt-4"
        style={{
          paddingBottom: SHELF_TOP_HEIGHT,
          position: 'relative',
          zIndex: 3,
        }}
      >
      ...
      {/* ✅ STRONGER, SOFTER WALL SHADOW BELOW SHELF */}
      <div
        ...
        style={{
          ...
          height: 22,
          background:
            'linear-gradient(to bottom, rgba(0,0,0,0.45), rgba(0,0,0,0.20), rgba(0,0,0,0))',
          zIndex: 1,
          pointerEvents: 'none',
        }}
      />
...
export function LibraryPage() {
  ...
  const shelves = useMemo(() => {
    ...
    for (let i = 0; i < games.length; i += BOOKS_PER_SHELF) {
      rows.push(games.slice(i, i + BOOKS_PER_SHELF))
    }
      ...
      <div className="flex justify-between">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="pl-9 input-field w-48"
          />
        </div>
      </div>
